<template>
    <div class="slide_area">
        <ul class="reply_list">
            <li class="reply_item" v-for="(replyItem,replyIndex) in commentList" v-if="replyIndex<2"
                v-clickoutside="handleClose"
                v-long-tap="{fun:blackShowFun,param:{index:0,replyIndex:replyIndex,userId:replyItem.sendUserId}}" flex="cross:center">
                <div class="reply_wrapper"
                     @click.stop="replyComment(commentList[replyIndex].secId,commentList[replyIndex].appId,replyItem.sendUserId,'回复 '+replyItem.sendUserName+':')">
                    ...
                    <div class="delWrapper" v-if="blackShow && blackIndex==0 && blackReplyIndex==replyIndex">
                        <div class="delInner" flex="cross:center">
                            ...
                            <div flex-box="1" flex="main:center cross:center" class="copyBtn"
                                 @click="delComment(replyIndex,replyItem.tid)">删除
                            </div>
                        </div>
            ...
            <transition name="fade">
                <ul class="reply_list" v-if="isSlide">
                    <li class="reply_item" v-for="(replyItem,replyIndex) in commentList"
                        ...
                        <div class="reply_wrapper"
                            ...
                            <div class="delWrapper"
                                 v-if="blackShow && blackIndex==0 && blackReplyIndex==replyIndex">
                                <div class="delInner" flex="cross:center">
                                    <div flex-box="1" flex="main:center cross:center" class="copyBtn"
        ...
        <div class="reply_slide" flex="cross:center" @click="replySlide()"
             v-if="commentList&&commentList.length>2">
            <span class="span" v-if="!isSlide">展开</span>
            <span class="span" v-if="isSlide">收起</span>
            <span class="slideIcon" :class="{'slideIconTop':isSlide}"></span>
        </div>
    </div>
</template>
            ...
            delComment(replyIndex, tid,userId){
                if(userId==this.userId){
                    ...
                    API.delComment(param).then((res)=> {
                        if (res.code == 200) {
                            this.commentList.splice(replyIndex, 1);
                            this.$nextTick(function(){
                                this.$emit('toParentComponents', 'refresh')
                            })
                        }
