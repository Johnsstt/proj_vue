<template>
    <div class="container" ref="container"  v-cloak>
        <ul class="list" v-refresh="refresh" v-load="loadMore">
            <div class="refresh">
            ...
            <li class="list_item" v-for="(item,index) in videoList">
                <div class="list_main" @click.stop="toDetail(item.tid)">
                    ...
                    <div class="assign_homework" v-if="item.pices&&item.pices.length>0">
                        <img :src="imagedomain+item.pices[0].imgUrl+'?imageView2/0/w/702/format/jpeg'" v-if="item.pices[0].type==1">
                        <img :src="imagedomain+item.pices[0].imgUrl+'?vframe/jpeg/offset/1/w/702'" v-if="item.pices[0].type==3">
                        <span class="num" flex="main:center cross:center" v-if="item.pices.length>0" v-text="item.pices.length"></span>
                    ...
                    <transition name="fade">
                        <div v-if="item.isSlide">
                            <ul class="slided" flex>
                                <li class="slided_item" flex-box="0" v-for="(pic,m) in item.claimPics" @click="clientSwiper(item.claimPics,m)">
...
<script type="text/babel">
    ...
    export default{
        ...
        methods: {
            ...
            toDetail(tid){
                if(this.role=='1'){
                    this.$router.push({path: '/app/opus/detail', query: {tid: tid, classId: this.classId}})
                }else{
                    this.$router.push({path: '/app/opus/studentList', query: {tid: tid, classId: this.classId}})
                }
            },
            /*
             * @description: 客户端浏览文件
            ...
            refresh () {
                ...
                API.getOpusList(param).then(res=> {
                    let {code,rs}=res;
                    if (code == 200) {
                        if (rs.pageList) {
